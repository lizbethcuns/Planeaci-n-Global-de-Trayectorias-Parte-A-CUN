...
# ================= REMUESTREO CONFIGURABLE =================
def resample_path(path_map, resolution, origin, step_meters):
    if len(path_map) < 2:
        return path_map

    path_world = [
        map_to_world(x, y, resolution, origin, None)
        for x, y in path_map
    ]
    ...
    for i in range(1, len(path_world)):
        ...
        while acc_dist + seg_len >= step_meters:
            ratio = (step_meters - acc_dist) / seg_len
            xn = x0 + ratio * dx
            yn = y0 + ratio * dy
...
if __name__ == "__main__":
    ...
    ax.set_aspect('equal')
    ax.set_title(f"Planificación LPA* (F1Tenth) - Waypoints cada {REQUIRED_SPACING}m")
    ...
    if not path_map:
        ...
        # 1. Dibujar ruta base (Línea ROJA continua)
        px, py = zip(*path_map)
        ax.plot(px, py, 'r-', linewidth=2, alpha=0.6, label='Ruta Original')
        
        # 2. GENERAR Y DIBUJAR WAYPOINTS (Puntos AZULES)
        final_waypoints = resample_path(
            path_map,
            resolution,
            origin,
            REQUIRED_SPACING 
        ...
        # Convertir a pixeles para dibujar
        wx_pixels = []
        wy_pixels = []
        for p in final_waypoints:
            pix = world_to_map(p[0], p[1], resolution, origin)
            wx_pixels.append(pix[0])
            wy_pixels.append(pix[1])
            
        ax.plot(wx_pixels, wy_pixels, 'bo', markersize=4, label=f'Waypoints ({REQUIRED_SPACING}m)')
        ax.legend() 

        # Volver a pintar Inicio/Fin 
        ...
        # Guardar CSV
        filename = f"lpastar_{str(REQUIRED_SPACING).replace('.', '')}m.csv"
        save_path_world_csv(final_waypoints, filename)
        print(f"Ruta LPA* guardada → {filename}")
        
        print("-" * 30)
        print(f"🔹 Waypoints Generados: {len(final_waypoints)} (Separación {REQUIRED_SPACING}m)")
        print(f"⏱️ Tiempo de cálculo: {end_time - start_time:.4f} s")
        print("-" * 30)
