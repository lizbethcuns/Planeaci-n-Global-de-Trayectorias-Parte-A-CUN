...
class LPAStar:
    ...
    def compute_shortest_path(self):
        ...
        while self.U and iter_count < max_iterations:
            ...
            # --- ANIMACIÓN DE CELDAS EXPLORADAS (Gris Sólido) ---
...
# ================= REMUESTREO 1 METRO (CAMBIO AQUÍ) =================
def resample_path_1m(path_map, resolution, origin, image_height):
    if len(path_map) < 2:
        return path_map
...
if __name__ == "__main__":
    ...
    if not path_map:
        ...
        # Guardar CSV con 1 METRO de separación
        path_1m = resample_path_1m(
            path_map,
            resolution,
            origin,
            map_bin.shape[0]
        )
        save_path_world_csv(path_1m, "lpastar_1m.csv")
        print(f"Ruta LPA* guardada → lpastar_1m.csv")
        
        # =======================================================
        # REPORTE FINAL
        # =======================================================
        REQUIRED_SPACING = 1.0 # <--- CAMBIO VISUAL PARA EL PRINT
        final_waypoints = path_1m

        print("-" * 30)
        print(f"🔹 Waypoints: {len(final_waypoints)} (Separación ~{REQUIRED_SPACING}m)")
