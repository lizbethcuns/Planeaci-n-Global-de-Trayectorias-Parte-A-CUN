...
Es importante resaltar que en este trabajo en tiempo de ejecucion para el LPA* fue mucho mas corto que la ejecucion del rrt , ya que el LPA* solo demora segundos en generarse el mapa mientras que el rrt demoro aproximadamente 2 minutos ya sea para la separacion de 0.5m y de 1m , uno de los problemas que presento fue que en el rrt no logró completar la trayectoria hasta el objetivo final, generando únicamente una ruta corta y parcial. Esto se debió principalmente a la reducción de resolución del mapa (downsampling) y a la binarización, que provocaron la pérdida de detalles en la pista y una representación excesiva de obstáculos. Como consecuencia, el espacio libre disponible para la expansión del árbol fue limitado, impidiendo que el RRT explorara completamente el entorno y alcanzara el objetivo. El resultado fue una solución local válida, pero no una trayectoria global completa.
